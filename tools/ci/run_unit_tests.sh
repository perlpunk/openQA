...
sudo zypper addrepo -G https://download.opensuse.org/repositories/network:chromium/openSUSE_Tumbleweed/network:chromium.repo
sudo zypper refresh
sudo zypper -n install chromium chromedriver
rpm -qa | grep chrom
